...
void enableMotor(int nodeId) {
    ...
    delay(1); // Wait for the state transition; adjust delay as needed
...
void reset_error(int motor_id){
  SDO_write(motor_id,0x2b,0x6040, 0x00, 0x0080);   // 0x6040 Controlword:  0x0080 Reset Faults
  SDO_write(motor_id,0x2b,0x6040, 0x00, 0x0000);   // 0x6040 Controlword:  0x0000 Soft stop
}
...
int calculatePositionInPulses(double desiredAngle, int pulsesPerFullRotation, int homingPulsesOffset, double transmissionRatio) {
    // Adjust the desired angle based on the transmission ratio
    double motorShaftDegrees = desiredAngle * transmissionRatio;

    // Convert the desired motor shaft rotation from degrees to pulses
    long desiredPositionInPulses = (motorShaftDegrees * pulsesPerFullRotation) / 360.0;

    // Adjust the position based on the homing offset to get the final position in pulses
    int adjustedPositionInPulses = desiredPositionInPulses + homingPulsesOffset;

    return adjustedPositionInPulses;
}
...
double calculatePositionInDegrees(int pulsesPerFullRotation, int homingPulsesOffset, double transmissionRatio, int currentPositionInPulses) {
    // Adjust the current position based on the homing offset
    int adjustedPositionInPulses = currentPositionInPulses - homingPulsesOffset;

    // Calculate the motor shaft rotation in degrees
    double motorShaftDegrees = (360.0 * adjustedPositionInPulses) / pulsesPerFullRotation;

    // Adjust the position based on the transmission ratio
    double outputShaftDegrees = motorShaftDegrees / transmissionRatio;
